"""Main PyReVanced application class."""
...
class ReVancedGUI:
    ...
    def setup_logging(self):
        ...
        if self.config_manager.save_logs_enabled:
            logging.info(f"PyReVanced v{self.__version__} started - Logs saved")
        else:
            logging.info(f"PyReVanced v{self.__version__} started - Console only")
    ...
    def update_preferences(self):
        ...
        # Handle config saving toggle
        old_config_enabled = self.config_manager.save_config_enabled
        self.config_manager.save_config_enabled = self.config_var.get()
        
        # Only save config if it's still enabled, or if we're turning it off (final save)
        if self.config_manager.save_config_enabled or (old_config_enabled and not self.config_manager.save_config_enabled):
